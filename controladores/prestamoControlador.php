...
class prestamoControlador extends prestamoModelo{
	...
	public function agregar_prestamo_controlador() {
		...
		if ($_SESSION['prestamo_item']==0) {
			$alerta=[
				"Alerta"=>"simple",
				"Titulo"=>"Ocurrió un Error Inesperado",
				"Texto"=>"No has Seleccionado Ningún Item para realizar el Préstamo",
				"Tipo"=>"error"
			];
			echo json_encode($alerta);
			exit();
	...
	public function paginador_prestamos_controlador($pagina,$registros,$privilegio,$url,$tipo,$fecha_inicio,$fecha_final){
		...
		if ($total>=1 && $pagina<=$Npaginas) {
			$tabla.='<p class="text-right">Mostrando Préstamos de '.$reg_inicio.' al '.$reg_final.' de un total de '.$total.'</p>';
	...
	public function agregar_pago_controlador() {
		...
		if ($datos_prestamo->rowCount()<=0) {
			$alerta=[
				"Alerta"=>"simple",
				"Titulo"=>"Ocurrió un Error Inesperado",
				"Texto"=>"El Prestamo al cuál  Intenta Agregar el PAGO NO Existe en el Sistema",
				"Tipo"=>"error"
			];
			echo json_encode($alerta);
			exit();
	...
	public function actualizar_prestamo_controlador() {
		...
		if (prestamoModelo::actualizar_prestamo_modelo($datos_prestamo_up)) {
			$alerta=[
				...
				"Texto"=>"Los Datos del Préstamo se han Actualizado con Éxito",
				"Tipo"=>"success"
			];
		} else {
			$alerta=[
				"Alerta"=>"simple",
				"Titulo"=>"Ocurrió un Error Inesperado",
				"Texto"=>"No Hemos Podido Actualizar el Préstamo",
				"Tipo"=>"error"
			];
		}
		echo json_encode($alerta);
